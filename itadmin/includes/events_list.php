...
function deleteMessage(u_id){
	if(confirm("Are you sure to delete this ?")){
		document.location.href="EventsAction.php?act=del&id="+u_id;
	}
}
</script>
 <?php
$total_page = $eventsObj->getTotalPages(); 
$page = isset($_GET['page'])?(int)$_GET['page']:0;
$orderby = isset($_SESSION[SES]['events']['orderby'])?$_SESSION[SES]['events']['orderby']:"id";
$order = isset($_SESSION[SES]['events']['order'])?$_SESSION[SES]['events']['order']:"desc";
$res = $eventsObj->getAllEvents($page*PAGE_LIMIT,$orderby, $order); 
$total = $res['NO_OF_ITEMS'];
?>
<div class="content_header">
  <div class="heading flleft">Events</div>
  <div class="heading flright"><a href="<?=$_curpage?>?q=add">Add New Events</a></div>
...
<?php if($total>0) {?>
<form action="EventsAction.php" method="post" >
...
?>
    <tr>
      ...
      <td width="31" height="25" align="center"><?php if($status=="1") {  ?>
        <a href="EventsAction.php?act=disable&id=<?=$id?>"><img src="images/enable.png" border="0" alt="Click to Disable" title="Disable"/></a>
        <?php
		} elseif ($status=="0") { ?>
        <a href="EventsAction.php?act=enable&id=<?=$id?>"><img src="images/disable.png" border="0" alt="Enable" title="Click to Enable"/></a>
        <?php
		} ?>
      </td>
	  ...
	  <input type="checkbox" name="new" onclick="chkboxEL(this)" value="<?=$id?>" <?php if($new==1) echo 'checked="checked"';?>/>
	  <span id="newBlink<?=$id?>">
