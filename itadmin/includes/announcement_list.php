...
function deleteMessage(u_id){
	if(confirm("Are you sure to delete this ?")){
		document.location.href="announcementAction.php?act=del&id="+u_id;
	}
}
</script>
 <?php
$total_page = $announcementObj->getTotalPages(); 
$page = isset($_GET['page'])?(int)$_GET['page']:0;
$orderby = isset($_SESSION[SES]['announcement']['orderby'])?$_SESSION[SES]['announcement']['orderby']:"id";
$order = isset($_SESSION[SES]['announcement']['order'])?$_SESSION[SES]['announcement']['order']:"desc";
$res = $announcementObj->getAllAnnouncement($page*PAGE_LIMIT,$orderby, $order); 
$total = $res['NO_OF_ITEMS'];
?>
<div class="content_header">
  <div class="heading flleft">Upcoming Course</div>
  <div class="heading flright"><a href="<?=$_curpage?>?q=add">Add Upcoming Course</a></div>
  <div class="flright"><select name="page" onchange="document.location.href='news.php?page='+this.value" class="sel1"><?php
...
<?php if($total>0) {?>
<form action="announcementAction.php" method="post" >
...
?>
    <tr>
      ...
      <td width="31" height="25" align="center"><?php if($status=="1") {  ?>
        <a href="announcementAction.php?act=disable&id=<?=$id?>"><img src="images/enable.png" border="0" alt="Click to Disable" title="Disable"/></a>
        <?php
		} elseif ($status=="0") { ?>
        <a href="announcementAction.php?act=enable&id=<?=$id?>"><img src="images/disable.png" border="0" alt="Enable" title="Click to Enable"/></a>
        <?php
		} ?>
      </td>
	  ...
	  <input type="checkbox" name="new" onclick="chkboxAL(this)" value="<?=$id?>" <?php if($new==1) echo 'checked="checked"';?>/>
	  <span id="newBlink<?=$id?>">
