...
$res = $noticeObj->getNoticeById($id); 
...
<div class="content_header">
  <div class="heading flleft">Update Notice</div>
...
<div class="bodycontent">
  <form name="frmnotice" action="NotificationAction.php" method="post" onsubmit="return notice_valid()" enctype="multipart/form-data">
    <input type="hidden" name="act" value="updatenotice" />
    ...
    <div id="validation_div" class="validation_error" align="center"></div>
      <table width="100%" border="0" cellspacing="0" cellpadding="3" align="center">
        ...
        <tr>
          ...
          <td>
		  <textarea name="title" style="width:99%;height: 50px;"  id="title" onblur="fillURL(this.value,'url')"><?=outText($res['oDATA'][0]['title']);?></textarea>
		  </td>
        </tr>
        <tr>
          ...
          <td align="center">:</td>
          <td>
	<input class="two" type="text" name="publish_date" value="<?=outText($res['oDATA'][0]['publish_date']);?>" readonly=""/>
	</td>
        </tr>
         <tr>
          <td colspan="3">
		  <div id="desc" <?=$display?>>
		   <b>URL <?=$required?></b><br />
		  <input type="text" name="url" id="url" value="<?=outText($res['oDATA'][0]['url']);?>" size="70" maxlength="20"/>
		...
		<div id="file" <?=$display1?>>
			...
			<?php if($res['oDATA'][0]['file_name']) {?><a href="../notices-file/<?=outText($res['oDATA'][0]['file_name']);?>" target="_blank">view</a><?php }?>
