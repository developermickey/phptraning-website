...
<div class="innerDiv" id="calendar">
 <marquee align="absmiddle" direction="up" scrollamount="1" truespeed="truespeed" scrolldelay="45" onMouseOver="this.stop();" onMouseOut="this.start();" height="180">
<ul>
<?php
	for($i=0;$i<$total;$i++){
		$id = ($res['oDATA'][$i]['id']);
		...
		<?php }?>
		<?php if($type == 0){?>
		<li><span><?=$pdate?></span><a href="<?=newsUrl($url)?>" class="newslinkk13" ><?=$title?></a>
		<?php if($new ==1){?><img src="<?=SITE_HOME?>images/new.gif"  border="0" align="absmiddle"/><?php }?>
		</li>
		<?php }
		}
