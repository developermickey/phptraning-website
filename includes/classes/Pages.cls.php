...
class Pages extends SiteData {
	...
	function addPage($request){
		...
		if($res['NO_OF_ITEMS']>0) {
			...
			$sql = "INSERT INTO ".PAGES." (page_id, page_name, page_url, page_type, page_updated,image) VALUES (NULL, '$page_name', '$page_url', '$page_type', '$page_updated','$img')";
	...
	function editPage($request){
	  ...
	  if(($_FILES['img']['type'] != "image/jpeg") && ($_FILES['img']['type'] != "img/png") && ($_FILES['image']['type'] != "img/jpg") && ($_FILES['img']['type'] != "img/PNG") && ($_FILES['img']['type'] != "image/JPG")&& ($_FILES['img']['type'] != "image/png") && ($_FILES['img']['type'] != "img/JPEG")){
		...
		if($res['NO_OF_ITEMS']>0) {
			...
			$sql = "UPDATE ".PAGES." set page_name='$page_name', page_url='$page_url', page_type='$page_type', page_updated='$page_updated',image='$img' where page_id='$page_id'";
			$res = $this->update($sql);
