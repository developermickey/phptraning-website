...
class PhotoGallerys extends SiteData {
	public $file_type = array("image/gif", "image/jpeg", "image/jpeg", "image/png", "image/bmp", "application/pdf"); 
	...
	function addPhoto($request){ 
		...
		if(isset($_FILES['photo_file'])) {	
			for($i=0;$i<count($_FILES['photo_file']['name']);$i++) {
				...
				if(($_FILES['photo_file']['type'][$i] != "image/jpeg") && ($_FILES['photo_file']['type'][$i] != "image/png") && ($_FILES['photo_file']['type'][$i] != "image/jpg") && ($_FILES['photo_file']['type'][$i] != "image/PNG") && ($_FILES['photo_file']['type'][$i] != "image/JPG") && ($_FILES['photo_file']['type'] [$i]!= "image/JPEG")){
	...
	function editPhoto($request) {
		...
		if( isset($_FILES['photo_file']['name']) && $_FILES['photo_file']['name']!="" ) {
				$notice = "nluo";
				if(($_FILES['photo_file']['type'] != "image/jpeg") && ($_FILES['photo_file']['type'] != "image/png") && ($_FILES['photo_file']['type'] != "image/jpg") && ($_FILES['photo_file']['type'] != "image/PNG") && ($_FILES['photo_file']['type'] != "image/JPG") && ($_FILES['photo_file']['type'] != "image/JPEG")){
			die("Sorry!!! Invalid Image Format Please Upload jpeg/png/jpg");
		}else{
